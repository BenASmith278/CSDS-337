...
public:
    ...
    ASTExpressionMultiplication(std::unique_ptr<ASTExpression> m1, std::unique_ptr<ASTExpression> m2) : m1(std::move(m1)), m2(std::move(m2)) {}

    // Create a new mulitplication expression.
    // m1: Left side of expression of the multiplication statement.
    // m2: Right side of expression of the multiplication statement.
    static auto Create(std::unique_ptr<ASTExpression> m1, std::unique_ptr<ASTExpression> m2)
    {
        return std::make_unique<ASTExpressionMultiplication>(std::move(m1), std::move(m2));
    }
